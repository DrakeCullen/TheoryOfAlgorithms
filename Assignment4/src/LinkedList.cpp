 ...
 */
template<typename T> 
void LinkedList<T>::updateNeighborsForShorterPath(bool visited[], PriorityQueue<T> &pq, int index, int distance[], int actualDistance[], int prev[])
{
	T* curr = head->next;
	...
	while (curr != tail) {
		if (!visited[curr->index])
		{
			int newDistance = distance[index] + curr->weight + curr->h;
			if (newDistance < distance[curr->index])
			{
				distance[curr->index] = newDistance;
				actualDistance[curr->index] = actualDistance[index] + curr->weight;
...
template<typename T> 
int LinkedList<T>::getHeuristic(string &destinationCity)
{
	T* curr = head->next;

	while (curr != tail) {
		if (curr->data == destinationCity)
			return curr->h;
		curr = curr->next;
	}
	return 0;
...
void LinkedList<T>::print()
{
	...
	while (curr != tail) {
		cout << curr->from << " to "<<curr->index << " " << curr->weight << ' ' << curr->h << endl;
		curr = curr->next;
	}
}
