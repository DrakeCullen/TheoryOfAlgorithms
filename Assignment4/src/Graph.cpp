...
void Graph::MST(int startIndex)
{
	...
	createMST(ordering, timeTaken, "Youngstown, OH");
 ...
 * O(1). Every operation is constant
 */
void Graph::addHeuristicEdge(int city1, int city2, string name, string destination, int weight) 
{ 
	LinkedList<City>* adjNeighbors = adjacencyList.getElement(city1);
	int h = adjNeighbors->getHeuristic(destination);
	...
	LinkedList<City>* mstNeighbors = mst.getElement(city1);
    mstNeighbors->push(newCity);
}
/**
 ...
 * O(V) where V is the number of cities in the graph
 */
void Graph::createMST(City ordering[], double timeTaken, string destination)
{
	for (int i = 1; i < currentSize; i++) 
	{
		addHeuristicEdge(ordering[i].from, ordering[i].index, cities.getElement(ordering[i].from)->data, destination, ordering[i].weight);
		addHeuristicEdge(ordering[i].index, ordering[i].from, cities.getElement(ordering[i].index)->data, destination, ordering[i].weight);
	}
	//cout << cities.getElement(ordering[i].from)->index << " to "<<ordering[i].index << " \"distance = " << ordering[i].weight << '"' << endl;
...
void Graph::dijkstra(int startIndex, int endIndex)
{
	...
	int distance[currentSize], actualDistance[currentSize];
	bool visited[currentSize];
	...
	while (pq.getSize() > 0)
	{
		...
		if (currentCity.index == endIndex) break;

		LinkedList<City>* cityNeighbors = mst.getElement(currentCity.index);
		...
		*/
    	cityNeighbors->updateNeighborsForShorterPath(visited, pq, currentCity.index, distance, actualDistance, prev);
	}
